    ...
    in {
      packages = with pkgs; rec {
        silhouette = stdenvNoCC.mkDerivation rec {
          name = "silhouette";
          src = filtered-src;
          propagatedBuildInputs = [ coreutils bash (texlive.combine {
            inherit (pkgs.texlive) scheme-minimal latex-bin collection-luatex latexmk
            standalone infwarerr ltxcmds grfext kvdefinekeys kvoptions kvsetkeys
            xkeyval tools fontspec epstopdf-pkg pgf;
          }) ];
